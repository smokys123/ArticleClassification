...
def train(config, dataset):
    ...
    # result.csv
    current_path = os.path.dirname(os.path.abspath(__file__))
    result_path = os.path.join(current_path, 'result', 'result.csv')
    # writer.writeheader()
    ...
    model.embedding.weight.data.copy_(
        torch.from_numpy(pretrained_weight.astype(np.float32)))
    print("loading word2vec is done!")

    # doc_dataset =
    # (class, number of sentences, [number of words], [w1, w2, ... ])
    ...
    for epoch in range(300):
        ...
        print("Train dataset -- Epoch : {}, LOSS : {}".format(epoch,
                                                              train_loss/1000))
        ...
        with open(result_path, 'a') as csvfile:
            writer = csv.DictWriter(csvfile, ['epoch',
                                              'train_loss', 'train_accu',
                                              'test_loss', 'test_accu'])
            result.append({'epoch': epoch,
                           'train_loss': train_loss/1000,
                           'train_accu': train_accu/10,
                           'test_loss': test_loss/200,
                           'test_accu': test_accu/2})
